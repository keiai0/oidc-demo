...
| パスワード変更 | 認証情報の管理はOPの責務 |
| アカウントロックアウト | ブルートフォース攻撃対策はOPの責務 |
| Consent（同意）管理 | RP への情報提供承認は OIDC 仕様の範囲 |
| ACR/AMR クレーム | 認証強度の通知はOPの責務 |
| SSO（シングルサインオン） | 複数RPへの認証委譲 |
| SLO（シングルログアウト） | SSOの対になる責務 |
| ユーザーセルフサービス | メールアドレス変更・MFA管理・セッション管理 |
...
| JWT署名鍵管理 | 鍵のローテーション、失効、自動ローテーション |
| セキュリティインシデント対応 | 全トークン失効、全鍵失効 |
| 監査ログ | 認証イベントの構造化ログ出力 |
| ヘルスチェック / メトリクス | 運用監視（Liveness / Readiness / Prometheus） |
