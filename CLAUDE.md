...
- 1テーブル1ファイル
- **カラム追加・変更は既存のマイグレーションファイルを直接編集する**（ALTER TABLE の新規ファイルを作らない）。学習用プロジェクトであり本番運用しないため、段階的マイグレーションは不要
...
- exported な関数・型・インターフェースには godoc コメントを付ける
- deps.go のインターフェースメソッドには用途・引数の意味を記載する
- 「なぜそうしているか」が自明でない箇所にはコメントを付ける（RFC 条項の参照など）
- getter/setter、単純な委譲、名前から明らかに意味が分かるものにはコメント不要
